...
{
    public class DlxOccupancyModelHelper
    {
        public static void CreateModelPipeline(MLContext context, string outputModelPath = "dlxoccupancy_fastTreeTweedie.zip")
        {
            ConsoleWriteHeader("Training deluxe room occupancy prediction model");
        ...
        public static void Evaluate(MLContext context, string outputModelPath = "dlxoccupancy_fastTreeTweedie.zip")
        {
            ConsoleWriteHeader("Evaluate deluxe room occupancy Model");
        ...
        private static IList<OccupancyTrainer> GetOccupancyData()
        {
            ...
            {
                var dblist = context.RoomTypeOccupancy.Where(i => i.RoomTypeId == 11).ToList();
        ...
        private static float GetNextDayValue(DateTime date)
        {
            ...
            {
                date = date.AddDays(1);
                var item = context.RoomTypeOccupancy.Where(i => i.Date == date && i.RoomTypeId == 11).FirstOrDefault();

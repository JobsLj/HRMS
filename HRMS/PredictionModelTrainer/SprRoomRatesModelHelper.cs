...
{
    public class SprRoomRatesModelHelper
    {
        public static void CreateModelPipeline(MLContext context, string outputModelPath = "sprRate_fastTreeTweedie.zip")
        {
            ConsoleWriteHeader("Training SPR prediction model");
        ...
        public static void Evaluate(MLContext context, string outputModelPath = "sprRate_fastTreeTweedie.zip")
        {
            ConsoleWriteHeader("Evaluate SPR Model");
            ...
            Console.WriteLine($"Date: {dataSample.Date}, date to predict: 09-12-2018, - Real value: 728701, Predicted Forecast: {prediction.Score}");
        ...
        private static IList<Rates> GetRoomRates()
        {
            ...
            {
                var dblist = context.RoomRates.Where(item => item.RoomTypeId == 7).GroupBy(i => i.Date).ToList();

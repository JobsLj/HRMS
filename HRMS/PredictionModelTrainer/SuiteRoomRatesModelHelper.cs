...
{
    public class SuiteRoomRatesModelHelper
    {
        public static void CreateModelPipeline(MLContext context, string outputModelPath = "suiteRate_fastTreeTweedie.zip")
        {
            ConsoleWriteHeader("Training SUITE prediction model");
        ...
        public static void Evaluate(MLContext context, string outputModelPath = "suiteRate_fastTreeTweedie.zip")
        {
            ConsoleWriteHeader("Evaluate SUITE Model");
            ...
            Console.WriteLine($"Date: {dataSample.Date}, date to predict: 09-12-2018, - Real value: 1436787, Predicted Forecast: {prediction.Score}");
        ...
        private static IList<Rates> GetRoomRates()
        {
            ...
            {
                var dblist = context.RoomRates.Where(item => item.RoomTypeId == 10).GroupBy(i => i.Date).ToList();

...
{
    public class FamOccupancyModelHelper
    {
        public static void CreateModelPipeline(MLContext context, string outputModelPath = "famoccupancy_fastTreeTweedie.zip")
        {
            ConsoleWriteHeader("Training family room occupancy prediction model");
        ...
        private static IList<OccupancyTrainer> GetOccupancyData()
        {
            ...
            {
                var dblist = context.RoomTypeOccupancy.Where(i => i.RoomTypeId == 9).ToList();
        ...
        private static float GetNextDayValue(DateTime date)
        {
            ...
            {
                date = date.AddDays(1);
                var item = context.RoomTypeOccupancy.Where(i => i.Date == date && i.RoomTypeId == 9).FirstOrDefault();
